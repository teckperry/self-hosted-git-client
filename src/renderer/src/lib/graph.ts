...
export interface GraphRow {
  ...
  passing: { col: number; color: string }[]
}
...
type Lane = { target: string; color: string } | null
  ...
  for (const c of commits) {
    ...
    lanesAbove.forEach((l, col) => {
      if (l && l.target === c.hash) {
        incoming.push({ fromCol: col, toCol: myCol, color: l.color })
      }
    })
    ...
    if (c.parents.length > 0) {
      lanesBelow[myCol] = { target: c.parents[0], color }
      outgoing.push({ fromCol: myCol, toCol: myCol, color })
      for (let i = 1; i < c.parents.length; i++) {
        const pcol = firstNull(lanesBelow)
        const pcolor = nextColor()
        lanesBelow[pcol] = { target: c.parents[i], color: pcolor }
        outgoing.push({ fromCol: myCol, toCol: pcol, color: pcolor })
      }
    } else {
    ...
    const passing: { col: number; color: string }[] = []
    lanesAbove.forEach((l, col) => {
      if (l && l.target !== c.hash && lanesBelow[col] && lanesBelow[col]!.target === l.target) {
        passing.push({ col, color: l.color })
      }
    })
    ...
    rows.push({ hash: c.hash, col: myCol, color, incoming, outgoing, passing })
